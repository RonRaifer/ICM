...
public class MainController implements clientConnectionIF {
	...
	private ClientConnector client;			// product database handler.
	private int srvPort;
	...
	public void EstablishConnection(ActionEvent event) {
		try {
		    	...
		    	client.handleMessageFromClientUI(aList);
	...
	@FXML
	public void onQuitAppClick(ActionEvent event) {
    	try {
	...
	private void onOptionsClick(ActionEvent event) {
		...
		try {
			...
			option.setDetails(srvHost, srvPort, client);
	...
	@FXML
	private void onStartClick(ActionEvent event) throws Exception {
